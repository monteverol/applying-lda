...
const Movies = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://127.0.0.1:5000/api/get-playlists?type=${type}`)
    ...
    const updatedPlaylists = playlists.map((playlist) =>
      playlist.name === selectedPlaylist.name
        ? {
            ...
            items: [
              ...playlist.items,
              { title: newItemTitle, year: new Date().getFullYear() },
            ],
          }
        : playlist
    );
  ...
  const handleGenerate = (playlistName, newItems) => {
    ...
    axios
      .post("http://127.0.0.1:5000/api/save-playlist", newPlaylist)
  ...
  return (
    <div className="movies snap-y snap-mandatory h-screen w-full overflow-y-scroll">
      ...
      <div className="h-screen w-full snap-center p-16 flex flex-col gap-8">
        <h1 className="text-[#CFCFCF] text-6xl font-bold">
          Generated Playlists: Movies
        </h1>
